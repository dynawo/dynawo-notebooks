model MyBESS_initialized_with_PRef_input "WECC BESS with REEC-C and REGC-B with a plant controller REPC-A on infinite bus"
  ...
  Modelica.Blocks.Interfaces.RealInput PRefPu(start = 0.5) "Active power reference in pu (base SNom)" annotation(
    Placement(visible = true, transformation(origin = {90, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  Modelica.Blocks.Interfaces.RealOutput PInjRefPu_out "Active power reference sent from REPC-A to REEC-C in pu (base SNom)";
...
equation
  ...
  connect(PRefPu, BESS.PRefPu) annotation(
...
end MyBESS_initialized_with_PRef_input;

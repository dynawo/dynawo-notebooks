...
class ModelicaParser:
    ...
    def parse_topology(self) -> Dict[str, Dict]:
        ...
        """
        logger.info("Starting topology parsing pipeline...")

        # 1. Raw Extraction
        components_raw = self.conn.get_components(self.model_name)
        logger.info(f"Raw components found: {len(components_raw) if components_raw else 0}")
        ...
        logger.info(f"Global System Bases detected: Sn={global_sn} MVA, Un={global_un} kV")

        # 4. Component Iteration
        if components_raw:
            for i, comp in enumerate(components_raw):
                c_type, c_name = comp[0], comp[1]
                mods = self.conn.get_component_modification(self.model_name, i + 1)
                self._process_component(c_name, c_type, mods, topo, global_sn, global_un)

        # 5. Connectivity Resolution
        raw_connections = self.conn.get_connections(self.model_name)
        data = self._resolve_connectivity(topo, raw_connections)
    ...
    def _get_param(self, comp: str, param: str, mods: str) -> Optional[str]:
        """Tries OMC evaluation first (for numbers), falls back to Regex (for refs)."""
    ...
    def _resolve_connectivity(self, data: Dict, connections: List) -> Dict:
        ...
        logger.info("Mapping physical connections to node-breaker topology...")
        ...
        def ensure_bus(comp_id: str) -> str:
            ...
            if vbus not in data["buses"]:
                ...
                logger.info(f" > Created Virtual Bus '{vbus}' to anchor component '{comp_id}'")
            return vbus
        ...
        for c1_raw, c2_raw in connections:
            ...
            if line_id:
                ...
                shared_bus = ensure_bus(c1)
                self._connect_to_bus(data, c1, shared_bus)
                self._connect_to_bus(data, c2, shared_bus)
    ...
    def _merge_init_blocks(self, data: Dict) -> Dict:
        ...
        logger.info("Merging logic INIT blocks into physical Generators...")
        ...
        for gid, info in gens.items():
            if info.get("is_init_block"):
                ...
                if host and host in gens:
                    ...
                    logger.info(f" > Merged '{gid}' (Type: {info.get('type')}) into host '{host}'")
                    to_remove.append(gid)
                else:
                    logger.warning(
                        f" > Orphan INIT block '{gid}' skipped (Host '{host}' not found or invalid)."
                    )

...
def correctVoltageRegulatorOn(filename):
    iidm, iidm_root, iidm_namespace, iidm_prefix_root, iidm_namespace_uri, iidm_prefix_root_string = loadXml(filename)
    ...
    with open(filename, 'wb') as doc:
        doc.write(etree.tostring(iidm_root, pretty_print = True, xml_declaration = True, encoding='ISO-8859-1', standalone=False))

def parameterValues(par_path, dyd_path):
    par, par_root, par_namespace, par_prefix_root, par_namespace_uri, par_prefix_root_string = loadXml(par_path)
    dyd, dyd_root, dyd_namespace, dyd_prefix_root, dyd_namespace_uri, dyd_prefix_root_string = loadXml(dyd_path)
    parameters = {}
    for element in par_root:
        if not isinstance(element, etree._Comment):
            if 'set' in element.tag:
                setId = element.attrib['id']
                blackBox = dyd.xpath("//*[local-name() = 'blackBoxModel'][@parId = '" + setId + "']")
                if blackBox:

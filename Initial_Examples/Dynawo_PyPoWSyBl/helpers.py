...
def loadXml(filename):
    ...
    xml_prefix_root_string = (
        "{" + str(xml_namespace_uri) + "}" if xml_prefix_root in xml_namespace else ""
    )
...
def defineParameters(modelName):
    ...
    (
        ...
        desc_xml_root,
        desc_xml_namespace,
        desc_xml_prefix_root,
        desc_xml_namespace_uri,
        desc_xml_prefix_root_string,
    ) = loadXml(dynawo_ddb + "/" + modelName)
...
def add_tap_changer_blocking(
    model_mapping, dynamic_model_id, tfo_ids, measurement_id, parameter_set_id=None
):
    ...
    tfo_df = pd.DataFrame.from_records(
        ...
        data={
            "dynamic_model_id": [dynamic_model_id for tfo in tfo_ids],
    ...
    model_mapping.add_tap_changer_blocking_automation_system(
        df=tcb_df, tfo_df=tfo_df, mp1_df=measurement1_df
    )
        ...
        for parameterName, parameterType in parameters_type.items():
            ...
            if parameterName in reference_parameters:
                    ...
                    if regulName in model_name:
                        if parameterName in self.regulation_values[regulName]:
                            if "TablesFile" in parameterName:
                                ...
                                parameter_attrib["value"] = (
                                    str(self.regulation_values[regulName][parameterName]).lower()
                                    if isinstance(
                                        self.regulation_values[regulName][parameterName],
                                        bool,
                                    )
                                    else str(self.regulation_values[regulName][parameterName])
                                )
                ...
                if parameterName == "generator_PNomAlt":
                    ...
                    parameterName == "generator_SNom"
                    or parameterName == "generator_SnTfo"
                    or parameterName == "transformer_SNom"
                ):
                ...
                if nominal_v > 30:
                    if parameterName in self.other_parameters_values:
                        parameter_attrib["value"] = str(
                            self.other_parameters_values[parameterName]
                        )

                    if (
                        parameterName == "generator_UBaseHV"
                        or parameterName == "generator_UNomHV"
                        or parameterName == "transformer_UBaseHV"
                        or parameterName == "transformer_UNomHV"
                    ):
                        parameter_attrib["value"] = str(nominal_v)
                    if withTfo:
                        if parameterName == "auxHV_P0Pu":
                            parameter_attrib["value"] = str(
                                0.005 * equipment_properties["max_p"] / 100
                            )
                        elif parameterName == "auxHV_Q0Pu":
                            parameter_attrib["value"] = str(
                                0.005 * equipment_properties["max_q_at_p"] / 100
                            )

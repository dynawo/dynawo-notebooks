    ...
    for generatorId, generatorProperties in generators.iterrows():
        if "GFM" in generatorId:
            if abs(generatorProperties["p"]) > 0.01:
                model_mapping.add_grid_forming_converter(
                    static_id=generatorId,
                    parameter_set_id="GFM",
                    model_name="GridFormingConverterDroopControl",
                )
        elif "EOL" in generatorId:
            if abs(generatorProperties["p"]) > 0.01:
                model_mapping.add_wecc(
                    static_id=generatorId,
                    parameter_set_id="Wind",
                    model_name="WTG4AWeccCurrentSource",
                )
